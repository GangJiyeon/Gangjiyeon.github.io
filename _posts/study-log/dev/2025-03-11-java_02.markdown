...
(2) 클래스(class): 사용자가 정의한 객체를 생성하는 데이터 타입<br>
• 사용자가 직접 정의한 데이터 타입이자, 새롭고 복합적인 데이터 타입<br>
• 프로그램: 클래스 정의들의 모음<br>
...
**(3) if-else if-else 문**<br>
• 여러 개의 조건을 검사할 때 사용<br>
...
**(4) switch 문**<br>
• 여러 개의 조건을 비교할 때 사용 (`==` 비교만 가능)<br>
• `case` 값과 비교하여 일치하는 경우 실행  <br>
• `break`를 사용하여 실행 종료 (없으면 다음 `case`까지 실행됨)<br>
