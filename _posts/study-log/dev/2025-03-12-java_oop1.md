...
**[ 생성자 사용 예시 ]**<br>
`생성자 체이닝`
> 하나의 생성자 안에서 자기 클래스의 다른 생성자를 호출하는 것: by `this(...)`<br>
• 생성자끼리만 호출 가능<br>
...
|static methods|객체 없이도, 클래스 이름으로 호출가능<br>used without object instantiation<br>1) can invoke only other static methods<br>2) can access only static variables<br>3) cannot use this/super|
