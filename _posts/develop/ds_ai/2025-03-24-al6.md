...
`주장`<br>
서비스 시간이 짧은 순서대로(non-decreasing order) 작업을 정렬하면 전체 체류 시간이 최소가 됨


`증명 요지: (귀류법)`<br>

• $i$번째 작업과 $i+1$번째 작업이 있다고 하자.
• 만약 $t_i > t_{i+1}$ 라면, 순서를 바꾸는 것이 더 나은 스케줄이 된다.
• 스케줄을 바꾸면 전체 체류 시간은 다음처럼 줄어든다:

$T' = T + t_{i+1} - t_i \quad \Rightarrow \quad T' < T$

• 즉, 기존 스케줄이 최적이라는 가정과 모순되므로, 작업은 반드시 $t_1 \le t_2 \le \dots \le t_n$ 순서여야 함

---

## 🧠 PART 2: Scheduling with Deadlines
...
**핵심 전략:**
- 작업을 profit 기준으로 정렬 (내림차순)
- 가능한 경우, 하나씩 스케줄에 삽입 (가능한 순서인지 체크)
...
```pseudo
S = ∅
sort jobs by nonincreasing profit

while (instance not solved):
    select next job
    if S is feasible with this job:
        add to S
if no more jobs:
    instance solved
```
...
| Feasible Sequence | Feasible Set |
|-------------------|--------------|
| [1, 3], [3, 1]     | {1, 2}       |
| [2, 1]            | {2, 3}       |
| [2, 3]            | {1, 4}       |
...
### 📌 6. Feasibility 정의 (정렬 기준)

> S is feasible **iff** the sequence obtained by ordering jobs in S by **nondecreasing deadlines** is feasible.

**논리:**  
작업 x가 y보다 먼저 배정되어 있고, y의 deadline이 더 빠를 경우,  
순서를 바꿔도 여전히 deadline을 만족하므로 feasible한 순서를 유지할 수 있음.

→ Exchange Sort를 통해 정렬된 순서가 feasible하다면 전체도 feasible.

---

### 📌 7. 최악 시간 복잡도 (Worst-case time complexity)

- 작업 정렬: \( \Theta(n \log n) \)
- 각 단계마다 최대 \( i \)개의 비교 필요  
- 전체 비교: \( \sum_{i=2}^{n} (i - 1 + i) = n^2 - 1 \in \Theta(n^2) \)
