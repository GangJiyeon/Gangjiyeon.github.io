...
//불가능
int 3Chapter; class if { } char false; void null() { } class %calc { } // 식별자의 첫문자로 숫자 사용 불가
// 자바의 예약어 if 사용 불가
// false 사용 불가
// null 사용 불가
// '%'는 특수문자
...
– (10장에서 상세히 공부한다)
– 여러 클래스의 공통된 기능/인터페이스를 표준화하는 방법
(~요구 스펙)
...
– String msg1, msg2;
– msg1 = “Hello”;
• 내용 변경이 안 된다. 자르고 늘일 수도 없다.
– 필요하면 새로운 객체로 만들어야 한다.
• ‘+’ 연산자 = 스트링 접합/연결-> 새 객체를 생성
– msg2 = msg1 + “World!”;
// msg2 = msg2 + “World!”
– System.out.println (“Salute: “ + msg2);
// -> “Salute: HelloWorld!” - 제 3의 객체
...
long: 2L
float: 3.14f
double: 2.0, 2.0E4 // = 2.0´104 = 20,000.0
char: ‘K’, ‘\n’, ‘\t’ // escape characters, \u000a \u0009
boolean: true, false
String : "Man is mortal" // 글자를모아만든객체 , !=배열
```
...
**[ enum(나열형) ]**
유한한 종류/가짓수의, 일종의 상수형 자료를 새로이 정
의한다
– 식별자/identifier 처럼 보이기도 하고
– String (문자열)처럼 출력되기도 하지만
– 내부적으로 정수로 취급된다
• Defined at two levels -> examples!
1. class × interface level에서 정의– global?
2. 변수/method 수준의 정의– member?
...
**[ 포장자(Wrapper) 클래스 ]**
• ‘포장자’ 클래스
– Primitive data type 데이터를 객체로 처리할 필요가 있을
때 객체로 만들어(‘포장해’) 줄 수 있는 클래스
...
| 변수 종류|설명 |
|:---:|---|
| **Class Variables**         | **클래스 변수** – `static` 변수로, 클래스 전체에서 공유|
| **Instance Variables**      | **인스턴스 변수** – 각 객체마다 개별적으로 존재하는 변수|
| **Local Variables**         | **지역 변수** – 메서드나 블록 내에서 선언되어 그 범위 안에서만 유효한 변수|
| **Array Elements**          | **배열 요소** – 배열의 각 인덱스에 저장된 값|
| **Method Arguments**        | **메서드 인자** – 메서드 호출 시 전달되는 값|
| **Constructor Arguments**   | **생성자 인자** – 생성자 호출 시 전달되는 값|
| **Exception Handler Arguments** | **예외 처리 인자** – 예외 처리 블록(`catch`)에서 전달받는 예외 객체입니다. |
