...
Car mycar;      //2. 자료형의 변수 선언(어딘가)

mycar = new Car();  // 3. 객체 생성

mycar.name = "붕붕이";  // 4. 문자열(객체) 생성 및 할당, 컴파일 시 문자열 객체 생성
````
...
• 비교: `==`(참조변수의 값, 같은 객체인가, 같은 메모리 주소), `.equals()` (두 객체의 내용이 같은가, 메모리 주소는 달라도 됨)
